/**
 * SyncClipboard Android 自动同步脚本 v3.1.1-2026.02.23 (SyncAutojs6.js)
 ...
 * 已实现无感同步剪贴板、文件、截图、通知等功能，支持 Android 16 等系统，需配合 Shizuku 和 AutoJs6 使用，无需 Root 权限
 *
 * 功能说明：
...
function uploadGroupItem(dirName, dirPath, zipName, zipBytes, groupHash) {
    ...
    return axios({ method: 'get', url: apiUrl, headers: { 'authorization': authHeader }, responseType: 'json' })
        .then(function(res) {
...
function doUploadFile(name, fullPath, mtime, bytes, fileHash, encodedName, profileType, removeAfterUpload, isScreenshot) {
    ...
    try {
        ...
        try {
            res = http.request(fileApiBaseUrl + encodedName, {
                method: 'PUT',
                headers: { 'authorization': authHeader },
            ...
            try {
                ...
                console.log('http.request direct body failed, trying axios: ' + e2.message)
            }
        }
...
function downloadTextFile(dataName) {
    ...
    try {
        const res = http.get(fileApiBaseUrl + encoded, {
            headers: { 'authorization': authHeader },
            timeout: 10000,
        })
        if (!res || res.statusCode < 200 || res.statusCode >= 300) {
